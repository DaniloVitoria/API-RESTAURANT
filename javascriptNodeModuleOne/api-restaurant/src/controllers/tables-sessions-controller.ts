...
class TablesSessionsController{
     ...
     async create(request: Request, response: Response, next: NextFunction){
          ...
          const session = await knex<TablesSessionsRepository>("tables_sessions").where({
               table_id,
            
          }).orderBy("opened_at", "desc").first()

          // se a mesa já esta aberta ele não pode abrir a mesma mesa aberta de novo
      ...
      async update(request: Request, response: Response, next: NextFunction){
          ...
          try {
               ...
               //Verifica se a seção fá foi fechada
